...
class BagWithFrequency:
    ...
    def iterator(self):
        return BagWithFrequenciesIterator(self)

class BagWithFrequenciesIterator:
    ...
    def valid(self):
        return self.__currentPos < len(self.__bag._BagWithFrequency__elems)

    def next(self):
        if self.__currentPos == len(self.__bag._BagWithFrequency__elems):
            raise ValueError()
        if self.__currentFr < self.__bag._BagWithFrequency__frequencies[self.__currentPos]:
            self.__currentFr += 1
        else:
            self.__currentPos += 1
            self.__currentFr = 1
    ...
    def getCurrent(self):
        if self.__currentPos == len(self.__bag._BagWithFrequency__elems):
            raise ValueError()
        return self.__bag._BagWithFrequency__elems[self.__currentPos]
