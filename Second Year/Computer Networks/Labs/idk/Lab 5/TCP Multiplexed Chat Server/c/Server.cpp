...
int main(int argc, char **argv){
    ...
    for(;;) {
        ...
        // selects modifies the list [&read_fds] and let's in the list only the ones we can't connect to
        ...
        for(i = 0; i <= fdmax; i++) {
          if (FD_ISSET(i, &read_fds)) {
            ...
            if (i == listener) {
              ...
              // send message from client to every client
              // recv => number of bytes we receive
              if ((nbytes = recv(i, buf, sizeof(buf), 0)) <= 0) {
               ...
               client_count--;
               close(i); // bye!
               FD_CLR(i, &master); // remove the socket from master set
              } else {
