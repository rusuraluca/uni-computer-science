...
import 'package:fitness_tracker/screens/fitness_data_page.dart';
...
class _MainSectionState extends State<MainSection> {
  ...
  saveFitnessData(FintessData fitnessData) async {
    if (!mounted) return;
    setState(() {
      isLoading = true;
    });
    if (online) {
      try {
        final FintessData received =
            await ApiService.instance.addFitnessData(fitnessData);
        DatabaseHelper.addFitnessData(received);
      } catch (e) {
        logger.e(e);
        message(context, "Error connecting to the server", "Error");
      }
